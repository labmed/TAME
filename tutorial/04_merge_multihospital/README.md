...
- 예: `성별` vs `sex`, `나이` vs `ageYears`, `보고값` vs `resultValue`
- 태그는 동일하므로 `tametools`는 `SEX`, `AGE`, `RESULT` 의미로 병합한다
...
```bash
tametools columns tutorial/04_merge_multihospital/merged_promote.tame
```
...
- 컬럼명이 서로 달라도 `SEX`, `AGE`, `RESULT`, `TESTNAME` 같은 태그 표준명으로 합쳐진다
...
```bash
tametools eda tutorial/04_merge_multihospital/merged_harmonize.tame --comparator-policy HARMONIZE
```
